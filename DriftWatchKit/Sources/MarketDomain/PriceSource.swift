...
/// source can be swapped in one line. Kept minimal on purpose; more methods
...
public protocol PriceSource: Sendable {
    ...
    /// reading it must not wait on the source's actor (see concurrency rule).
    ///
    /// Contract: exactly ONE consumer per source. AsyncStream is unicast,
    ...
    /// finishes only when the source is done for good (disconnected or,
